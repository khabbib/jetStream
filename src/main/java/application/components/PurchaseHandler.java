                            ...
                            if (!month.isEmpty()) {
                                if (!cvc.isEmpty()) {
                                    ...
                                    if (validCard) {
                                        ...
                                        if (controller.departure_seat != null) {
                                            for (int i = 0; i <= 1; i++) {
                                                ...
                                                if (controller.departure_seat != null && controller.return_seat != null) { // saving tur flight
                                                    ...
                                                    StringBuilder rfc = controller.connection.generateRandomRFC();
                                                    String date = LocalDate.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));

                                                    boolean saveTicket = controller.connection.savePurchasedTicket(controller.user.getUserId(), controller.return_seat, String.valueOf(rfc), date, controller.departure_seat, false);
                                                    if (saveTicket) {
                                                    ...
                                                    boolean saveTicket = controller.connection.savePurchasedTicket(controller.user.getUserId(), controller.booking_flight_number_lbl.getText(), String.valueOf(rfc), date, controller.booking_seat_number_lbl.getText(), false);
                                                    if (saveTicket) {
                                                        purchaseDone1 = true;
                                                        rfc1 = String.valueOf(rfc);
                                        ...
                                        controller.confirmActions.displayMessage(controller.payment_err_msg, "Card is not valid.", true);
                                    }
                                } else {
                                    controller.confirmActions.displayMessage(controller.payment_err_msg, "CVC is empty!", true);
                                }
                            } else {
                                controller.confirmActions.displayMessage(controller.payment_err_msg, "Month is empty!", true);
                            }
                        } else {
                            controller.confirmActions.displayMessage(controller.payment_err_msg, "Year is empty!", true);
                        }
                    } else {
                        controller.confirmActions.displayMessage(controller.payment_err_msg, "Last name is empty!", true);
                    }
                } else {
                    controller.confirmActions.displayMessage(controller.payment_err_msg, "First name is empty!", true);
                }
            }else {
                controller.confirmActions.displayMessage(controller.payment_err_msg, "Card numbers is empty!", true);
            }
                    ...
                    if (fourdigit.length() == 12) {
                        if (email.length() >= 6 && email.length() <= 60 && email.contains("@") && email.contains("gmail")) {
                            if (!seat.isEmpty()) {
                               ...
                               controller. confirmActions.displayMessage(controller.booking_msg_lbl, "Please choose a seat!", true);
                            }
                        } else {
                            controller.confirmActions.displayMessage(controller.booking_msg_lbl, "Email char 6-30 or format issue!", true);
                        }
                    } else {
                        controller.confirmActions.displayMessage(controller.booking_msg_lbl, "SSN shall be 12 chars!", true);
                    }
                } else {
                    controller.confirmActions.displayMessage(controller.booking_msg_lbl, "Lastname shall be 3-30 chars!", true);
                }
            }else {
                controller.confirmActions.displayMessage(controller.booking_msg_lbl, "Firstname shall be 3-30 chars!", true);
            }
